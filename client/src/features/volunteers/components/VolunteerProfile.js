...
export function VolunteerProfile(props) {
    ...
    useEffect(() => {
        if (updateSuccess) {
            setFeedbackDisplay(
                <StatusSnackbar
                    statusTitle={"Volunteer Saved Successfully"}
    ...
    useEffect(() => {
        if (isUpdating) {
            setFeedbackDisplay(
                <StatusSnackbar
                    statusTitle={"Saving Volunteer... Do Not Refresh Page"}
                    anchorVertical={'bottom'}
                    anchorHorizontal={'right'}
                    statusType={"info"}
                />
    ...
    useEffect(() => {
        if (updateError) {
            setFeedbackDisplay(
                <StatusSnackbar 
                    statusTitle={"Volunteer Save Failed!"} 
    ...
    if (volunteerLoading || fetchedAwardsLoading || fetchedRolesLoading || fetchedDocumentsLoading || fetchedTrainingLoading || upcomingAwardsLoading || upcomingTrainingLoading) {
        return (<Box maxWidth="md" sx={{ m: 2, p: 3, margin: "auto" }}>
            <ProfileSkeleton />
        </Box>)
    } else if (volunteerSuccess || passedVolunteer) {
        return (
